<x-layouts.app :title="'Create Category'">
    <h1 class="text-3xl font-bold mb-6 dark:text-white">Create Category</h1>
        ...
        <form action="{{ route('admin.categories.store') }}" method="POST" class="space-y-4">
            ...
            <div>
                ...
                <textarea name="description" rows="4"
                    class="mt-1 w-full rounded-lg border p-2 dark:border-zinc-700 dark:bg-zinc-800">{{ old('description') }}</textarea>
            </div>

            <div class="flex gap-2">
                <a href="{{ route('admin.categories.index') }}" class="px-4 py-2 border rounded">Cancel</a>
                <button class="px-4 py-2 bg-blue-600 text-white rounded">Save Category</button>
            </div>
        </form>
    </div>
</x-layouts.app>
